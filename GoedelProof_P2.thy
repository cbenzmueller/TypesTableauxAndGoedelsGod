...
section \<open>Second Part -  God's existence is necessary if possible\<close>
...
theorem T1: "\<lfloor>\<^bold>\<forall>X. \<P> X \<^bold>\<rightarrow> \<^bold>\<diamond>\<^bold>\<exists>\<^sup>E X\<rfloor>" using A1a A2 by blast  (** Positive properties are possibly instantiated*)  
...
abbreviation essenceOf::"\<up>\<langle>\<up>\<langle>O\<rangle>,O\<rangle>" ("\<E>") where "\<E> Y x \<equiv> (Y x) \<^bold>\<and> (\<^bold>\<forall>Z. Z x \<^bold>\<rightarrow> Y \<succeq> Z)"
abbreviation beingIdenticalTo::"O\<Rightarrow>\<up>\<langle>O\<rangle>" ("id") where "id x  \<equiv> (\<lambda>y. y\<^bold>\<approx>x)" (** id is a rigid predicate*)  
  
(** These two are invalid when using concept containment 3 & 4*)    
theorem GodIsEssential: "\<lfloor>\<^bold>\<forall>x. G x \<^bold>\<rightarrow> (\<E> G x)\<rfloor>" using A1b A4a by metis    (** Theorem 11.20 - Informal Proposition 5 *)
theorem "\<lfloor>\<^bold>\<forall>x. G* x \<^bold>\<rightarrow> (\<E> G* x)\<rfloor>" using A4a by meson                      (** Theorem 11.21 *)
    
(** Theorem 11.22 - Something can have only one essence *)
theorem "\<lfloor>\<^bold>\<forall>X Y z. (\<E> X z \<^bold>\<and> \<E> Y z) \<^bold>\<rightarrow> (X \<succeq> Y)\<rfloor>" by meson  
  
(** Theorem 11.23 - An essence is a complete characterization of an individual *)
theorem EssencesCharacterizeCompletely: "\<lfloor>\<^bold>\<forall>X y. \<E> X y \<^bold>\<rightarrow> (X \<succeq> (id y))\<rfloor>"
proof (rule ccontr)
  assume "\<not> \<lfloor>\<^bold>\<forall>X y. \<E> X y \<^bold>\<rightarrow> (X \<succeq> (id y))\<rfloor>"
  hence "\<exists>w. \<not>(( \<^bold>\<forall>X y. \<E> X y \<^bold>\<rightarrow> X \<succeq> id y) w)" by simp
  then obtain w where "\<not>(( \<^bold>\<forall>X y. \<E> X y \<^bold>\<rightarrow> X \<succeq> id y) w)" ..
  hence "(\<^bold>\<exists>X y. \<E> X y \<^bold>\<and> \<^bold>\<not>(X \<succeq> id y)) w" by simp
  hence "\<exists>X y. \<E> X y w \<and> (\<^bold>\<not>(X \<succeq> id y)) w" by simp
  then obtain P where "\<exists>y. \<E> P y w \<and> (\<^bold>\<not>(P \<succeq> id y)) w" ..
  then obtain a where 1: "\<E> P a w \<and> (\<^bold>\<not>(P \<succeq> id a)) w" ..
  hence 2: "\<E> P a w" by (rule conjunct1)
  from 1 have "(\<^bold>\<not>(P \<succeq> id a)) w" by (rule conjunct2)
  ...
  from 2 have "\<forall>Y. Y a w \<longrightarrow> ((P \<succeq> Y) w)" by auto
  hence "(\<^bold>\<rightharpoondown>(id b)) a w \<longrightarrow> (P \<succeq> (\<^bold>\<rightharpoondown>(id b))) w" by (rule allE)
  hence "\<not>(\<^bold>\<rightharpoondown>(id b)) a w \<or> ((P \<succeq> (\<^bold>\<rightharpoondown>(id b))) w)" by blast 
  then show False proof
    assume "\<not>(\<^bold>\<rightharpoondown>(id b)) a w"
    hence "a = b" by simp
    thus False using 4 by auto
    next  
    assume "((P \<succeq> (\<^bold>\<rightharpoondown>(id b))) w)"
...
proof -
{
  fix w 
  {
    ...
    have  "(\<^bold>\<forall>x. G x \<^bold>\<rightarrow> (\<E> G x)) w" using GodIsEssential by (rule allE)   (** GodIsEssential follows from Axioms 11.11 and 11.3B *)
...
lemma MC: "\<lfloor>\<^bold>\<forall>\<Phi>.(\<Phi> \<^bold>\<rightarrow> (\<^bold>\<box> \<Phi>))\<rfloor>" (*nitpick*) oops (** With S5 modal collapse is actually valid (see proof below)*)
...
theorem possibleExistenceImpliesNecEx: "\<lfloor>\<^bold>\<diamond>\<^bold>\<exists> G \<^bold>\<rightarrow> \<^bold>\<box>\<^bold>\<exists>\<^sup>E G\<rfloor>" (** Local consequence*)
proof -
  have "\<lfloor>\<^bold>\<exists> G \<^bold>\<rightarrow> \<^bold>\<box>\<^bold>\<exists>\<^sup>E G\<rfloor>" using GodExistenceImpliesNecExistence by simp (** Which follows from Axioms 11.11, 11.25 and 11.3B*)
  ...
  have 2: "\<lfloor>\<^bold>\<diamond>\<^bold>\<box>\<^bold>\<exists>\<^sup>E G \<^bold>\<rightarrow> \<^bold>\<box>\<^bold>\<exists>\<^sup>E G\<rfloor>" using symm tran by metis  (** Using S5 semantic frame conditions*)
...
lemma T4: "\<lfloor>\<^bold>\<diamond>\<^bold>\<exists> G\<rfloor> \<longrightarrow> \<lfloor>\<^bold>\<box>\<^bold>\<exists>\<^sup>E G\<rfloor>" using possibleExistenceImpliesNecEx by simp (** global consequence*)
...
(** Non-normal models (with Leibniz equality): Monotheism follows straightforward from God having all and only positive properties *)
...
(** Normal models are trickier. We need to consider some previous results (Page 162)*)

lemma GodExistenceIsValid: "\<lfloor>\<^bold>\<exists>\<^sup>E G\<rfloor>" using GodNecExists refl by auto  (** useful lemma *)
...
{
  fix w 
  have "\<lfloor>\<^bold>\<exists>\<^sup>E G\<rfloor>" using GodExistenceIsValid by simp   (** \<lfloor>\<^bold>\<exists>\<^sup>E G\<rfloor> follows from corollary 11.28 *)
  hence "(\<^bold>\<exists>\<^sup>E G) w" by (rule allE)       
  then obtain g where 1: "existsAt g w \<and> G g w" ..
  hence 2: "\<E> G g w" using GodIsEssential by blast  (** Theorem 11.20 follows from Axioms 11.11 and 11.3B *)
  {
    fix y
    have "G y w \<longleftrightarrow> (g \<^bold>\<approx> y) w" proof 
      assume "G y w"
      hence 3: "\<E> G y w" using GodIsEssential by blast      
      have "(\<E> G y \<^bold>\<rightarrow> (G \<succeq> id y)) w" using EssencesCharacterizeCompletely by simp (** Theorem 11.23 *)
      hence "\<E> G y w \<longrightarrow> ((G \<succeq> id y) w)" by simp
      from this 3 have "(G \<succeq> id y) w" by (rule mp) 
      ...
      have "w r w" using refl by simp               (** we rely explicitly on frame reflexivity (Axiom M) *)
...
lemma PosPropertiesNecExist: "\<lfloor>\<^bold>\<forall>Y. \<P> Y \<^bold>\<rightarrow> \<^bold>\<box>\<^bold>\<exists>\<^sup>E Y\<rfloor>" using GodNecExists A4a by meson (** Corollary 11.28 and A4a needed for the proof*)
 
    
subsection \<open>Objections & Criticism\<close>
  ...
  fix w
   {
    ...
    hence "(Q \<^bold>\<rightarrow> \<^bold>\<box>((\<^bold>\<exists>\<^sup>Ez. G z) \<^bold>\<rightarrow> Q)) w" by force  (** we can use rule 'useful' too*)
