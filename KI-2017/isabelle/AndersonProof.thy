...
(**In this section, we verify Anderson's emendation of G\"odel's argument @{cite "anderson90"},
 ...
 dropping axiom \emph{A1b} (@{text "\<lfloor>\<^bold>\<forall>X. \<^bold>\<not>(\<P> X) \<^bold>\<rightarrow> \<P> (\<rightharpoondown>X)\<rfloor>"}). As a consequence, he changes following definitions
...
(*>*)
  ...
  abbreviation Essence::"\<up>\<langle>\<up>\<langle>\<zero>\<rangle>,\<zero>\<rangle>" ("\<E>\<^sup>A") where
...
(*>*)   
  ...
  Q.E.D.*)
  theorem GodIsEssential: "\<lfloor>\<^bold>\<forall>x. G\<^sup>A x \<^bold>\<rightarrow> (\<E>\<^sup>A G\<^sup>A x)\<rfloor>" proof -
(*<*)
  {
    fix w
    {
          ...
          proof
              ...
              
              {
                ...
                have "(\<P> Q u) \<longrightarrow> (\<forall>x. G\<^sup>A x u \<longrightarrow> ((Q x)) u)" 
                  using ax_M by auto (**and using @{text "\<box>(\<phi> x) \<longrightarrow> \<phi> x"}*)
              }    
 ...
 qed
  ...
  theorem T4: "\<lfloor>\<^bold>\<exists> G\<^sup>A \<^bold>\<rightarrow>  \<^bold>\<box>\<^bold>\<exists>\<^sup>E G\<^sup>A\<rfloor>" proof -
  {
    fix w 
    {
...
(*>*)
  ...
  theorem T5: "\<lfloor>\<^bold>\<diamond>\<^bold>\<exists> G\<^sup>A\<rfloor> \<longrightarrow> \<lfloor>\<^bold>\<box>\<^bold>\<exists>\<^sup>E G\<^sup>A\<rfloor>" proof -
(*<*)      
    have "\<lfloor>\<^bold>\<exists> G\<^sup>A \<^bold>\<rightarrow> \<^bold>\<box>\<^bold>\<exists>\<^sup>E G\<^sup>A\<rfloor>" using T4 
    ...
    have 2: "\<lfloor>\<^bold>\<diamond>\<^bold>\<box>\<^bold>\<exists>\<^sup>E G\<^sup>A \<^bold>\<rightarrow> \<^bold>\<box>\<^bold>\<exists>\<^sup>E G\<^sup>A\<rfloor>" using ax_B ax_IV by metis
...
(*>*)
  ...
  lemma GodExistsNecessarily: "\<lfloor>\<^bold>\<box>\<^bold>\<exists>\<^sup>E G\<^sup>A\<rfloor>" using T3 T5 by metis
