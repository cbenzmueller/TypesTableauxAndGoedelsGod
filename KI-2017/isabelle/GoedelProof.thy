...
lemma "\<lfloor>\<^bold>\<not>(\<phi> \<Rrightarrow> \<chi>) \<^bold>\<rightarrow> \<^bold>\<diamond>\<^bold>\<exists>\<^sup>E \<phi>\<rfloor>" by auto (*possible instantiation of @{text "\<phi>"} assumed implicitly*) 
...
theorem EssencesCharacterizeCompletely: "\<lfloor>\<^bold>\<forall>X y. \<E> X y \<^bold>\<rightarrow> (X \<Rrightarrow> (id y))\<rfloor>"
  proof (rule ccontr) (**proof by contradiction not shown here*)
  (*<*)
...
lemma GodExistenceIsValid: "\<lfloor>\<^bold>\<exists>\<^sup>E G\<rfloor>" using GodNecExists refl by auto
...
lemma PosPropertiesNecExist: "\<lfloor>\<^bold>\<forall>Y. \<P> Y \<^bold>\<rightarrow> \<^bold>\<box>\<^bold>\<exists>\<^sup>E Y\<rfloor>" using GodNecExists A4a
  by meson (**follows from corollary 11.28 and axiom A4a*)
