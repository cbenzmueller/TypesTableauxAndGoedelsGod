...
(**G\"odel's particular version of the argument is a direct descendant of that of Leibniz, which in turn derives
 ...
 As we will see, the success of this argumentation depends on how we formalize our notion of entailment.*)
  ...
  lemma "\<lfloor>\<^bold>\<not>(\<phi> \<Rrightarrow> \<chi>) \<^bold>\<rightarrow> \<^bold>\<diamond>\<^bold>\<exists>\<^sup>A \<phi>\<rfloor>" by auto (**possible instantiation of @{text "\<phi>"} implicit*)
...
(**The definition of property entailment introduced by G\"odel can be criticized on the grounds that it lacks
 ...
 where the notion of \emph{concept containment} in contrast to ordinary \emph{property entailment} is discussed.}*)
  ...
  consts Positiveness::"\<up>\<langle>\<up>\<langle>e\<rangle>\<rangle>" ("\<P>") (**positiveness applies to intensional predicates*)
  abbreviation Existence::"\<up>\<langle>e\<rangle>" ("E!") (**object-language existence predicate*) 
    where "E! x  \<equiv> \<lambda>w. (\<^bold>\<exists>\<^sup>Ay. y\<^bold>\<approx>x) w"
...
(**G\"odel's axioms for the first part essentially say that (A1) either a property or its negation must be positive,
  ...
  abbreviation appliesToPositiveProps::"\<up>\<langle>\<up>\<langle>\<up>\<langle>e\<rangle>\<rangle>\<rangle>" ("pos") where
    "pos Z \<equiv>  \<^bold>\<forall>X. Z X \<^bold>\<rightarrow> \<P> X"  
  abbreviation intersectionOf::"\<up>\<langle>\<up>\<langle>e\<rangle>,\<up>\<langle>\<up>\<langle>e\<rangle>\<rangle>\<rangle>" ("intersec") where
...
(**Being Godlike is defined as having all (and only) positive properties.*)
  abbreviation God::"\<up>\<langle>e\<rangle>" ("G") where "G \<equiv> (\<lambda>x. \<^bold>\<forall>Y. \<P> Y \<^bold>\<rightarrow> Y x)"
  abbreviation God_star::"\<up>\<langle>e\<rangle>" ("G*") where "G* \<equiv> (\<lambda>x. \<^bold>\<forall>Y. \<P> Y \<^bold>\<leftrightarrow> Y x)"
...
(**Axiom \emph{A4a} and its consequence \emph{A4b} together imply that @{text "\<P>"} satisfies Fitting's
  ...
  lemma "\<lfloor>rigid \<P>\<rfloor>" using A4a A4b by blast
...
(**Something can have only \emph{one} essence.*)
...
(*<*) assume "\<not> \<lfloor>\<^bold>\<forall>X y. \<E> X y \<^bold>\<rightarrow> (X \<Rrightarrow> (id y))\<rfloor>"
    ...
    hence "\<exists>x. \<exists>z. w r x \<and>  z actualizedAt x \<and> P z x \<and> \<not>(a = z)" by blast
    then obtain w1 where "\<exists>z. w r w1 \<and>  z actualizedAt w1 \<and> P z w1 \<and> \<not>(a = z)" ..
    then obtain b where 3: "w r w1 \<and>  b actualizedAt w1 \<and> P b w1 \<and> \<not>(a = b)" ..
    hence "w r w1" by simp
    from 3 have "b actualizedAt w1" by simp
    ...
    then show False proof
      ...
      hence "\<forall>x. \<forall>z. (w r x \<and> z actualizedAt x \<and> P z x) \<longrightarrow> (\<^bold>\<rightharpoondown>(id b)) z x" by blast
      hence "\<forall>z. (w r w1 \<and> z actualizedAt w1 \<and> P z w1) \<longrightarrow> (\<^bold>\<rightharpoondown>(id b)) z w1" 
          by (rule allE)
      hence "(w r w1 \<and> b actualizedAt w1 \<and> P b w1) \<longrightarrow> (\<^bold>\<rightharpoondown>(id b)) b w1" by (rule allE)
      hence  "\<not>(w r w1 \<and> b actualizedAt w1 \<and> P b w1) \<or> (\<^bold>\<rightharpoondown>(id b)) b w1" by simp
...
(**G\"odel introduces a particular notion of \emph{necessary existence} as the property something has,
  ...
  abbreviation necessaryExistencePredicate::"\<up>\<langle>e\<rangle>" ("NE") 
...
(**(Possibilist) existence of God implies its necessary (actualist) existence.*)
  theorem T4: "\<lfloor>\<^bold>\<exists> G \<^bold>\<rightarrow> \<^bold>\<box>\<^bold>\<exists>\<^sup>A G\<rfloor>" proof - (**not shown*)
(*<*)
  {
    fix w 
    {
...
(*<*)
    ...
    have 2: "\<lfloor>\<^bold>\<diamond>\<^bold>\<box>\<^bold>\<exists>\<^sup>A G \<^bold>\<rightarrow> \<^bold>\<box>\<^bold>\<exists>\<^sup>A G\<rfloor>" using ax_B ax_IV by metis (**frame conditions*)
...
(*>*)
  ...
  theorem GodExistsNecessarily: "\<lfloor>\<^bold>\<box>\<^bold>\<exists>\<^sup>A G\<rfloor>" using T3 T5 by metis
  lemma GodExistenceIsValid: "\<lfloor>\<^bold>\<exists>\<^sup>A G\<rfloor>" using GodExistsNecessarily ax_T by auto
...
(**Monotheism for non-normal models (using Leibniz equality) follows directly from God having all
 ...
 (@{cite "Fitting"}, p. 162).*)
  ...
  lemma Monotheism_normal: "\<lfloor>\<^bold>\<exists>x.\<^bold>\<forall>y. G y \<^bold>\<leftrightarrow> x \<^bold>\<approx> y\<rfloor>" proof - (**not shown*)
(*<*)
  {
    fix w 
    have "\<lfloor>\<^bold>\<exists>\<^sup>A G\<rfloor>" using GodExistenceIsValid by simp
    hence "(\<^bold>\<exists>\<^sup>A G) w" by (rule allE)       
    then obtain g where 1: "g actualizedAt w \<and> G g w" ..
    hence 2: "\<E> G g w" using GodIsEssential by blast
    {
      fix y
      have "G y w \<longleftrightarrow> (g \<^bold>\<approx> y) w" proof 
        assume "G y w"
        hence 3: "\<E> G y w" using GodIsEssential by blast      
        have "(\<E> G y \<^bold>\<rightarrow> (G \<Rrightarrow> id y)) w" using EssencesCharacterizeCompletely by simp
        hence "\<E> G y w \<longrightarrow> ((G \<Rrightarrow> id y) w)" by simp
        from this 3 have "(G \<Rrightarrow> id y) w" by (rule mp) 
        hence "(\<^bold>\<box>(\<^bold>\<forall>\<^sup>Az. G z \<^bold>\<rightarrow> z \<^bold>\<approx> y)) w" by simp
        hence "\<forall>x. w r x \<longrightarrow> ((\<forall>z. (z actualizedAt x \<and> G z x) \<longrightarrow> z = y))" by auto
        hence "w r w \<longrightarrow> ((\<forall>z. (z actualizedAt w \<and> G z w) \<longrightarrow> z = y))" by (rule allE)
        hence "\<forall>z. (w r w \<and> z actualizedAt w \<and> G z w) \<longrightarrow> z = y" by auto
        hence 4: "(w r w \<and> g actualizedAt w \<and> G g w) \<longrightarrow> g = y" by (rule allE)
        have "w r w" using ax_T by simp
        hence  "w r w \<and> (g actualizedAt w \<and> G g w)" using 1 by (rule conjI)
...
(**Fitting @{cite "Fitting"} also discusses the objection raised by Sobel @{cite "sobel2004logic"}, 
  ...
  lemma ModalCollapse: "\<lfloor>\<^bold>\<forall>\<Phi>. \<Phi> \<^bold>\<rightarrow> \<^bold>\<box>\<Phi>\<rfloor>" proof -
   { fix w
     { fix Q
      have "(\<^bold>\<forall>x. G x \<^bold>\<rightarrow> (\<E> G x)) w" using GodIsEssential by (rule allE)
      hence "\<forall>x. G x w \<longrightarrow> (Q \<^bold>\<rightarrow> \<^bold>\<box>(\<^bold>\<forall>\<^sup>Az. G z \<^bold>\<rightarrow> Q)) w" by force
      hence 1: "(\<exists>x. G x w) \<longrightarrow> ((Q \<^bold>\<rightarrow> \<^bold>\<box>(\<^bold>\<forall>\<^sup>Az. G z \<^bold>\<rightarrow> Q)) w)" by (rule useful)
      have "\<exists>x. G x w" using GodExistenceIsValid by auto
      from 1 this have "(Q \<^bold>\<rightarrow> \<^bold>\<box>(\<^bold>\<forall>\<^sup>Az. G z \<^bold>\<rightarrow> Q)) w" by (rule mp)
      hence "(Q \<^bold>\<rightarrow> \<^bold>\<box>((\<^bold>\<exists>\<^sup>Az. G z) \<^bold>\<rightarrow> Q)) w" using useful by blast
      hence "(Q \<^bold>\<rightarrow> (\<^bold>\<box>(\<^bold>\<exists>\<^sup>Az. G z) \<^bold>\<rightarrow> \<^bold>\<box>Q)) w" by simp
