...
(**This work is divided in two parts. The first one introduces a shallow semantic embedding of
an intensional higher-order modal logic (abbr. IHOML) in a classical higher-order logic (Isabelle/HOL).
...
IHOML has not been automated before and has several applications e.g. for the deep semantic analysis of
natural language rational arguments. In this sense, our work contributes to the objectives
...
A study on \emph{computational metaphysics},
\footnote{This term was originally coined by Fitelson and Zalta (see @{cite "FitelsonZ07"}).
...
which has been criticized as an undesirable side-effect of the axioms postulated by G\"odel (resp. Scott), since it essentially  
states that there are no contingent truths and everything is determined.*)
(** 
...
In contrast to those, Fitting's variant is based on the use of an intensional as opposed
...
The work presented here originates from the \emph{Computational Metaphysics} lecture course held at the
...
(**We embed each modal operator as the set of worlds satisfying the corresponding HOL formula.*)
  ...
  abbreviation mnot::"io\<Rightarrow>io" ("\<^bold>\<not>_"[52]53) where "\<^bold>\<not>\<phi> \<equiv> \<lambda>w. \<not>(\<phi> w)"
  abbreviation mand::"io\<Rightarrow>io\<Rightarrow>io" (infixr"\<^bold>\<and>"51) where "\<phi>\<^bold>\<and>\<psi> \<equiv> \<lambda>w. (\<phi> w)\<and>(\<psi> w)"
  abbreviation mor::"io\<Rightarrow>io\<Rightarrow>io" (infixr"\<^bold>\<or>"50) where "\<phi>\<^bold>\<or>\<psi> \<equiv> \<lambda>w. (\<phi> w)\<or>(\<psi> w)"
  abbreviation mimp::"io\<Rightarrow>io\<Rightarrow>io" (infix"\<^bold>\<rightarrow>"49) where "\<phi>\<^bold>\<rightarrow>\<psi> \<equiv> \<lambda>w.(\<phi> w)\<longrightarrow>(\<psi> w)"
(*<*)
...
(*>*)      
(**Frame's accessibility relation and modal operators @{text "\<box>"} and @{text "\<diamond>"}.*)  
  ...
  abbreviation meqC:: "\<up>\<langle>\<up>\<zero>,\<up>\<zero>\<rangle>" (infixr"\<^bold>\<approx>\<^sup>C"52) (**equality for individual concepts*)
    where "x \<^bold>\<approx>\<^sup>C y \<equiv> \<lambda>w. \<forall>v. (x v) = (y v)"
  abbreviation meqL:: "\<up>\<langle>\<zero>,\<zero>\<rangle>" (infixr"\<^bold>\<approx>\<^sup>L"52) (**Leibniz equality for individuals*)
...
(**According to Fitting's semantics (@{cite "Fitting"}, pp. 92-4), @{text "\<down>"} is an unary operator applying only to 
 ...
 book \emph{Naming and Necessity}, where he discusses its many interesting ramifications in logics and the philosophy of language.}*)

(**Terms of the form @{text "\<down>\<alpha>"} are called \emph{relativized} (extensional) terms; they are always derived
from intensional terms and their type is extensional (in the color example @{text "\<down>r"} would be of type @{text "\<langle>\<zero>\<rangle>"}).
...
\footnote{In our computer-formalization and assessment of Fitting's textbook @{cite "J35"},
we provide further evidence that our embedded logic works as intended by verifying the book's theorems
and examples. We refer the reader to this work for further details.}*)
  ...
  \footnote{We prove our first theorem with Isabelle, as indicated by the keyword `by' followed by
  the method used for the proof. In this case Isabelle's simplifier (term rewriting) sufficed.
  Other proof methods used here are: \emph{blast} (tableaus), \emph{meson} (model elimination),
  ...
  (**@{text "\<beta>\<eta>"}-redex is valid for relativized terms as long as no modal operators occur.*)
...
subsection\<open>Stability, Rigid Designation, \emph{De Dicto} and \emph{De Re}\<close>
  ...
  lemma "\<lfloor>rigidPred (\<tau>::\<up>\<langle>\<zero>\<rangle>)\<rfloor> \<longleftrightarrow> \<lfloor>(stabilityA \<tau> \<^bold>\<and> stabilityB \<tau>)\<rfloor>" by meson   
  lemma "\<lfloor>rigidPred (\<tau>::\<up>\<langle>\<up>\<zero>\<rangle>)\<rfloor> \<longleftrightarrow> \<lfloor>(stabilityA \<tau> \<^bold>\<and> stabilityB \<tau>)\<rfloor>" by meson   
      
  (**\emph{De re} is equivalent to \emph{de dicto} for non-relativized (i.e. rigid) terms.
\footnote{The \emph{de dicto/de re} distinction is used regularly in the philosophy of language for disambiguation
