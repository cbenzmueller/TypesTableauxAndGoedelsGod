...
In this variant, Fitting chooses to reformulate these definitions using \emph{extensional} types (@{text "\<up>\<langle>\<langle>\<zero>\<rangle>\<rangle>"} and @{text "\<up>\<langle>\<langle>\<zero>\<rangle>,\<zero>\<rangle>"}) instead,
...
that they are honest (necessarily honest, in fact, because @{text "\<P>"} has also been proven rigid in this variant).*)
  ...
  abbreviation Essence::"\<up>\<langle>\<langle>\<zero>\<rangle>,\<zero>\<rangle>" ("\<E>") where
...
(*>*)
  ...
  lemma T4v1: "\<lfloor>\<^bold>\<exists> \<^bold>\<down>G \<^bold>\<rightarrow> \<^bold>\<box>\<^bold>\<exists>\<^sup>E \<^bold>\<down>G\<rfloor>" proof -
(*<*)
  {
    fix w 
    {
...
(*>*)
  ...
  lemma T4v2: "\<lfloor>\<^bold>\<exists> \<^bold>\<down>G \<^bold>\<rightarrow> ((\<lambda>X. \<^bold>\<box>\<^bold>\<exists>\<^sup>E X) \<^bold>\<down>G)\<rfloor>" using A4a T4v1 by metis
  ...
  lemma T5v1:"\<lfloor>\<^bold>\<diamond>\<^bold>\<exists> \<^bold>\<down>G\<rfloor>\<longrightarrow>\<lfloor>\<^bold>\<box>\<^bold>\<exists>\<^sup>E \<^bold>\<down>G\<rfloor>" using T4v1 T3deRe by metis
  lemma T5v2:"\<lfloor>(\<lambda>X. \<^bold>\<diamond>\<^bold>\<exists>\<^sup>E X) \<^bold>\<down>G\<rfloor> \<longrightarrow> \<lfloor>(\<lambda>X. \<^bold>\<box>\<^bold>\<exists>\<^sup>E X) \<^bold>\<down>G\<rfloor>" using T4v2 by blast
      
  (**Necessary Existence of God (\emph{de dicto} and \emph{de re} readings).*)    
  lemma GodNecExists_deDicto: "\<lfloor>\<^bold>\<box>\<^bold>\<exists>\<^sup>E \<^bold>\<down>G\<rfloor>" using T3deRe T4v1 by blast
  lemma GodNecExists_deRe: "\<lfloor>(\<lambda>X. \<^bold>\<box>\<^bold>\<exists>\<^sup>E X) \<^bold>\<down>G\<rfloor>" using T3deRe T5v2 by blast
