...
they validate the argument and we assume they correspond to Fitting's ideas.*)
...
(** In contrast to G\"odel's argument (as presented by Fitting), the following theorems can be proven in \emph{K} logic
 (note that the \emph{S5} axioms are no longer needed): *)
